...
//   }, [messages]);
  ...
  const handleCommandSubmit = async (command) => {
   ...
   { await sendPayloadToBackend(newMessages, command)
        ...
        const manualScrollButton = document.getElementById('manual-scroll-button')
...
//   },)
  ...
  return (
    <div className="App">
        {
            showChatButton && <div onClick = {handleChatButtonClick} style={{position: 'absolute', bottom: '40px', right: '20px', zIndex: '1000'}}>
                <img src={ChatButtonLogo} width="100px" height="100px" style={{cursor: "pointer"}} />
            </div>
        }
      {showChatPanel && <div style={{display: 'flex', flexDirection: 'column'}}>
        <div style={{ position:"absolute", height: "500px", width: "500px", bottom: "100px", right: "20px", padding:'20px'  }}>
      {/* <ArrowButton id="manual-scroll-button" direction="down" labelPosition="left" border onClick={() => handleManualScrollClick("auto")}>Scroll to bottom (force auto)</ArrowButton> */}
            <div style={{display: 'flex',alignItems: 'center',justifyContent: 'center',backgroundColor: 'black', color:'white',  padding: '10px'}}>
                <div style={{fontSize: '20px'}}>AJAFFE VA</div>
                <div onClick={handleChatButtonClose} style={{marginLeft: '60%', cursor: 'pointer', textDecoration: 'underline'}}>Close</div>
            </div>
        <MainContainer>
          <ChatContainer>       
            <MessageList 
              scrollBehavior="smooth" 
              typingIndicator={isTyping ? <TypingIndicator style={{marginTop:'20px', backgroundColor: 'transparent'}} content="Processing..." /> : null}
              style={{fontSize:'20px'}}
              autoScrollToBottom
          ...
          {/* <MessageInput style={{}} placeholder='dsds' /> */}
            ...
            <MessageInput id="chat-input" placeholder="Type message here" value={chatInputValue} onChange={(e)=>{setChatInputValue(e)}} onSend={handleCommandSubmit} 
